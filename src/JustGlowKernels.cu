    ...
    int boundMinX, int boundMinY, int boundWidth, int boundHeight)  // BoundingBox for Grid optimization
{
    ...
    // Physical decay weight (The Shape) - controlled by Falloff parameter
    // Level 0=level1Weight, then decay by pow(decayRate, level)
    float physicalWeight = calculatePhysicalWeight((float)levelIndex, decayK, falloffType, level1Weight);

    // Apply physical weight to current level's contribution
    // NOTE: Radius-based Soft Threshold removed (v1.5.4)
    //       - BoundingBox optimization already handles active area
    //       - Falloff parameter controls level weighting
    //       - Removed threshold was causing unwanted brightness loss
    ...
    // activeLimit (Radius) is now only used for MIP level limiting, not per-pixel threshold
    (void)activeLimit;  // Suppress unused parameter warning
